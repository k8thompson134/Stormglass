        ...
        <div className="font-mono text-[11px] leading-relaxed bg-[#0a0f1a] rounded-lg border border-[#1a2535] p-3 space-y-2 overflow-x-auto">
            ...
            <div>
                ...
                <div className="text-gray-300 pl-2">
                    <div>Pressure: <span className="text-white">{snapshot.pressure.toFixed(1)} hPa</span></div>
                    <div>Temperature: <span className="text-white">{toF(snapshot.temperature)}°F</span> <span className="text-gray-500">({snapshot.temperature.toFixed(1)}°C)</span></div>
                    <div>Humidity: <span className="text-white">{snapshot.humidity.toFixed(0)}%</span></div>
                    <div>Dew Point: <span className="text-white">{toF(snapshot.dewPoint)}°F</span></div>
                    <div>Wind: <span className="text-white">{snapshot.windSpeed.toFixed(1)} m/s</span></div>
                    <div>UV Index: <span className="text-white">{snapshot.uvIndex.toFixed(1)}</span></div>
                    <div>Cloud Cover: <span className="text-white">{snapshot.cloudCover.toFixed(0)}%</span></div>
                    <div>Precipitation: <span className="text-white">{snapshot.precipitation.toFixed(1)} mm</span></div>
                </div>
            </div>
            ...
            {snapshot.derivative && (
                <div>
                    ...
                    <div className="text-gray-300 pl-2">
                        <div>Δ1h: <span className="text-white">{snapshot.derivative.delta1h.toFixed(3)} hPa/h</span></div>
                        <div>Δ3h: <span className="text-white">{snapshot.derivative.delta3h.toFixed(3)} hPa/h</span></div>
                        <div>Δ6h: <span className="text-white">{snapshot.derivative.delta6h.toFixed(3)} hPa/h</span></div>
                        <div>Trend: <span className={
                            snapshot.derivative.trend === 'rising' ? 'text-red-400' :
                            snapshot.derivative.trend === 'falling' ? 'text-blue-400' :
                            'text-gray-400'
                        }>{snapshot.derivative.trend}</span></div>
                    </div>
                </div>
            )}
            ...
            {snapshot.aqi && (
                <div>
                    ...
                    <div className="text-gray-300 pl-2">
                        <div>US AQI: <span className="text-white">{snapshot.aqi.usAqi.toFixed(0)}</span></div>
                        <div>PM2.5: <span className="text-white">{snapshot.aqi.pm25.toFixed(1)} µg/m³</span></div>
                        <div>PM10: <span className="text-white">{snapshot.aqi.pm10.toFixed(1)} µg/m³</span></div>
                        <div>Ozone: <span className="text-white">{snapshot.aqi.ozone.toFixed(1)} ppb</span></div>
                        <div>NO₂: <span className="text-white">{snapshot.aqi.no2.toFixed(1)} ppb</span></div>
                        <div>SO₂: <span className="text-white">{snapshot.aqi.so2.toFixed(1)} ppb</span></div>
                        <div>CO: <span className="text-white">{snapshot.aqi.co.toFixed(1)} ppb</span></div>
                    </div>
                </div>
            )}
            ...
            {snapshot.geomagnetic && (
                <div>
                    ...
                    <div className="text-gray-300 pl-2">
                        <div>Kp Index: <span className="text-white">{snapshot.geomagnetic.kpIndex.toFixed(1)}</span></div>
                        <div>Solar Wind Speed: <span className="text-white">{snapshot.geomagnetic.solarWindSpeed.toFixed(0)} km/s</span></div>
                        <div>Solar Wind Density: <span className="text-white">{snapshot.geomagnetic.solarWindDensity.toFixed(1)} p/cm³</span></div>
                    </div>
                </div>
            )}
            ...
            {snapshot.pollen && (
                <div>
                    ...
                    <div className="text-gray-300 pl-2">
                        <div>Tree: <span className="text-white">{snapshot.pollen.treeIndex}/5</span></div>
                        <div>Grass: <span className="text-white">{snapshot.pollen.grassIndex}/5</span></div>
                        <div>Weed: <span className="text-white">{snapshot.pollen.weedIndex}/5</span></div>
                        <div>Mold: <span className="text-white">{snapshot.pollen.moldIndex}/5</span></div>
                    </div>
                </div>
            )}
        </div>
...
function LogEntry({ entry, onDelete }: { entry: SymptomLogEntry; onDelete: (id: string) => void }) {
    ...
    return (
        <div className="border border-[#1e2d45] rounded-xl bg-[#131d2e]/80 overflow-hidden">
            ...
            <div className="p-4 flex items-start gap-3">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border shrink-0 ${severityClass}`}>
                    {entry.severity}/10
                </span>
                <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-gray-400 font-mono">{formatTimestamp(entry.timestamp)}</p>
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                        {(entry.tags as string[]).map((tag, i) => (
                            <span key={i} className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-blue-500/15 text-blue-300 border border-blue-500/25">
                                {tag}
                            </span>
                        ))}
                    </div>
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-3">
                    ...
                    {!loading && logs.length === 0 && (
                        <div className="text-center py-12">
                            <p className="text-sm text-gray-400">No symptom logs in the last {days} days.</p>
                            <p className="text-xs text-gray-500 mt-1">Use the clipboard button in the header to log your first symptom.</p>
                        </div>
                    )}
                ...
                {!loading && logs.length > 0 && (
                    <div className="px-5 py-3 border-t border-gray-700/30 text-[10px] text-gray-500 font-mono">
                        {logs.length} {logs.length === 1 ? 'entry' : 'entries'} · last {days} days
                    </div>
                )}
            </div>
        </div>,
