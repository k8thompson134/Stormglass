...
export default function AQISeasonSummary({ sensitivity }: Props) {
  ...
  if (loading) {
    return (
      <div className="bg-gray-800/20 rounded-2xl h-[90px] animate-pulse" />
    );
  }
  ...
  const { totalDaysTracked, daysAtOrAboveThreshold, threshold, byCategory } =
    summary;
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap mb-3">
        <div>
          ...
          <p className="text-[13px] text-gray-300">
            <span
              className={`font-bold ${isNotable ? "text-amber-300" : "text-emerald-300"}`}
            >
        ...
        <>
          <div className="flex h-2 w-full rounded-full overflow-hidden bg-gray-800/60">
            {breakdown.map((b) => (
              <div
                key={b.category}
                style={{
                  width: `${(b.days / totalDaysTracked) * 100}%`,
                  backgroundColor: AQI_CATEGORY_THEME[b.category].dot,
                }}
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
            {breakdown.map((b) => (
              <span
                key={b.category}
                className={`text-[10px] font-mono ${AQI_CATEGORY_THEME[b.category].text}`}
              >
