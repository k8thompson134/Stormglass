...
import { weatherData, pressureDerivatives, symptomLogs, airQualityData, geomagneticData, pollenData, users } from '../db/schema.js';
...
export async function weatherRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Querystring: AQIForecastQuery }>('/api/weather/aqi-forecast', async (request, reply) => {
    const config = getCurrentConfig();
    const location = config ? `${config.latitude},${config.longitude}` : null;
    ...
    // range is requested.
    const historyHours = Math.max(1, Math.min(168, parseInt(request.query.hours || '6', 10) || 6));
    ...
    const chartUntil = new Date(now.getTime() + Math.min(historyHours, 72) * 60 * 60 * 1000);
    ...
    if (rows.length === 0) {
      return reply.status(404).send({ error: 'No air quality data available yet' });
    }
    ...
    const windowPoints = rows.map((r) => ({ timestamp: r.timestamp, usAqi: parseFloat(r.usAqi) }));
    ...
    const categoryCrossing = findNextCategoryCrossing(currentPoint.usAqi, futurePoints, now);
  ...
  app.get<{ Querystring: AQIBurdenQuery }>('/api/weather/aqi-burden', async (request, reply) => {
    const config = getCurrentConfig();
    const location = config ? `${config.latitude},${config.longitude}` : null;

    const days = Math.max(1, Math.min(365, parseInt(request.query.days || '90', 10) || 90));
    const parsedThreshold = parseInt(request.query.threshold ?? '', 10);
    const threshold = Math.max(0, Math.min(500, Number.isNaN(parsedThreshold) ? 100 : parsedThreshold));
    ...
    const rows = await db
      ...
      .from(airQualityData)
      .where(location
        ? and(eq(airQualityData.location, location), gte(airQualityData.timestamp, since), lte(airQualityData.timestamp, now))
        : and(gte(airQualityData.timestamp, since), lte(airQualityData.timestamp, now)))
      .orderBy(airQualityData.timestamp)
      .limit(10_000);

    if (rows.length === 0) {
      return reply.status(404).send({ error: 'No air quality history available yet' });
    }
    ...
    let timeZone = 'UTC';
    if (config) {
      const [userRow] = await db.select({ timezone: users.timezone }).from(users).where(eq(users.id, config.userId)).limit(1);
      if (userRow?.timezone) timeZone = userRow.timezone;
    }

    const summary = summarizeAqiBurden(
      rows.map(r => ({ timestamp: r.timestamp, usAqi: parseFloat(r.usAqi) })),
      threshold,
      timeZone
    );
