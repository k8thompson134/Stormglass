...
function makeUpdateChain() {
    const chain = {
        set: (values: Record<string, unknown>) => { lastUpdateSet = values; return chain; },
...
let restartResult: { userId: string; latitude: string; longitude: string; name?: string } | null = null;
vi.mock('../jobs/weather-poll.js', () => ({
    getCurrentConfig: vi.fn(() => ({ userId: USER_ID, latitude: '41.85003', longitude: '-87.65005' })),
...
describe('POST /api/settings/location', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        lastUpdateSet = null;
        restartResult = { userId: USER_ID, latitude: '41.85003', longitude: '-87.65005', name: 'Chicago, Illinois' };
    });
    ...
    it('persists a valid IANA timezone to the DB', async () => {
        ...
        const response = await app.inject({
            ...
            payload: { latitude: '41.85003', longitude: '-87.65005', name: 'Chicago, Illinois', timezone: 'America/Chicago' },
        });
    ...
    it('ignores an invalid timezone rather than persisting garbage or erroring', async () => {
        ...
        const response = await app.inject({
            method: 'POST',
            url: '/api/settings/location',
            payload: { latitude: '41.85003', longitude: '-87.65005', name: 'Chicago, Illinois', timezone: 'Not/AZone' },
        });
    ...
    it('omits timezone from the update entirely when none is provided, so an existing saved value is left untouched', async () => {
        ...
        const response = await app.inject({
            method: 'POST',
            url: '/api/settings/location',
            payload: { latitude: '41.85003', longitude: '-87.65005', name: 'Chicago, Illinois' },
        });
    ...
    it('still updates location and name even when the timezone is invalid', async () => {
        ...
        await app.inject({
            method: 'POST',
            url: '/api/settings/location',
            payload: { latitude: '41.85003', longitude: '-87.65005', name: 'Chicago, Illinois', timezone: 'garbage' },
        });
