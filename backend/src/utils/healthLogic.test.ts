...
describe("getMigraineRisk", () => {
  ...
  it("does not amplify or annotate when AQI is not elevated", () => {
    ...
    expect(
      clean.currentFactors.some((f) => f.toLowerCase().includes("air quality")),
    ).toBe(false);
  });
...
describe("getGeomagneticRisk", () => {
  ...
  it("low — Kp index below 4", () => {
    expect(
      getGeomagneticRisk({ kpIndex: 0, solarWindSpeed: 0, solarWindDensity: 0 })
        .risk,
    ).toBe("low");
    expect(
      getGeomagneticRisk({
  ...
  it("moderate — Kp index crosses 4", () => {
    expect(
      getGeomagneticRisk({ kpIndex: 4, solarWindSpeed: 0, solarWindDensity: 0 })
        .risk,
    ).toBe("moderate");
    expect(
      getGeomagneticRisk({
  ...
  it("high — Kp index crosses 5", () => {
    expect(
      getGeomagneticRisk({ kpIndex: 5, solarWindSpeed: 0, solarWindDensity: 0 })
        .risk,
    ).toBe("high");
    expect(
      getGeomagneticRisk({
  ...
  it("severe — Kp index crosses 7", () => {
    expect(
      getGeomagneticRisk({ kpIndex: 7, solarWindSpeed: 0, solarWindDensity: 0 })
        .risk,
    ).toBe("severe");
    expect(
      getGeomagneticRisk({ kpIndex: 9, solarWindSpeed: 0, solarWindDensity: 0 })
        .risk,
    ).toBe("severe");
  });

  it("includes Kp index in trigger label", () => {
    const result = getGeomagneticRisk({
      kpIndex: 6,
      solarWindSpeed: 450,
      solarWindDensity: 5,
    });
...
describe("getAQIRisk", () => {
  ...
  it("does not append PM2.5 N95 recommendation when not elevated", () => {
    ...
    expect(result.recommendations.some((r) => r.includes("N95 outdoors"))).toBe(
      false,
    );
  });
  ...
  describe("hyperlocal (PurpleAir) vs. regional model disagreement", () => {
    ...
    it("flags low confidence when hyperlocal is based on fewer than 3 sensors", () => {
      const oneS = getAQIRisk({
        ...base,
        usAqi: 80,
        hyperlocal: { usAqi: 90, pm25: 30, sensorCount: 1, nearestMiles: 2.0 },
      });
      expect(
        oneS.currentFactors.some((f) => f.includes("lower-confidence")),
      ).toBe(true);
      expect(
        oneS.currentFactors.some((f) => f.includes("Only 1 nearby sensor ")),
      ).toBe(true);

      const twoS = getAQIRisk({
        ...base,
        usAqi: 80,
        hyperlocal: { usAqi: 90, pm25: 30, sensorCount: 2, nearestMiles: 2.0 },
      });
      expect(
        twoS.currentFactors.some((f) => f.includes("Only 2 nearby sensors")),
      ).toBe(true);
    });
    ...
    it("does not flag low confidence with the usual 3-sensor average", () => {
      const result = getAQIRisk({
        ...base,
        usAqi: 80,
        hyperlocal: { usAqi: 90, pm25: 30, sensorCount: 3, nearestMiles: 0.5 },
      });
      expect(
        result.currentFactors.some((f) => f.includes("lower-confidence")),
      ).toBe(false);
    });
  });
    ...
    it("leads with a worsening-smoke warning including the forecast peak", () => {
      const result = getAQIRisk({
        ...
        smokeTrend: {
          ...
          currentPm25: 40,
          next24hPeakPm25: 120,
          next24hPeakUsAqi: 175,
          next24hPeakAt: null,
          likelyWildfireSmoke: true,
        },
      });
    ...
    it("adds an improving note without alarming language when trend is improving", () => {
      const result = getAQIRisk({
        ...
        smokeTrend: {
          ...
          next24hPeakUsAqi: 60,
          next24hPeakAt: null,
          likelyWildfireSmoke: false,
        },
      });
      expect(
        result.recommendations.some((r) => r.includes("trending better")),
      ).toBe(true);
    });
    ...
    it("adds neither message when trend is stable", () => {
      ...
      expect(
        result.recommendations.some(
          (r) => r.includes("trending worse") || r.includes("trending better"),
        ),
      ).toBe(false);
    });
  });
...
describe("getPOTSRisk", () => {
  ...
  it("elevated AQI adds a point and can tip low into moderate", () => {
    ...
    expect(
      getPOTSRisk(0, 50, 18, 130).currentFactors.some((f) => f.includes("130")),
    ).toBe(true);
  });
...
describe("getJointPainRisk", () => {
  ...
  it("elevated AQI adds a point and can tip low into moderate", () => {
    ...
    expect(
      getJointPainRisk(0.1, 50, 15, 130).currentFactors.some((f) =>
        f.includes("130"),
      ),
    ).toBe(true);
  });
...
describe("getPollenRisk", () => {
  ...
  it("low — all indices below 3", () => {
    expect(
      getPollenRisk({ treeIndex: 1, grassIndex: 2, weedIndex: 1, moldIndex: 0 })
        .risk,
    ).toBe("low");
  });
  ...
  it("moderate — any index reaches 3", () => {
    expect(
      getPollenRisk({ treeIndex: 3, grassIndex: 0, weedIndex: 0, moldIndex: 0 })
        .risk,
    ).toBe("moderate");
    expect(
      getPollenRisk({ treeIndex: 0, grassIndex: 0, weedIndex: 0, moldIndex: 3 })
        .risk,
    ).toBe("moderate");
  });
  ...
  it("high — any index reaches 4", () => {
    expect(
      getPollenRisk({ treeIndex: 0, grassIndex: 4, weedIndex: 0, moldIndex: 0 })
        .risk,
    ).toBe("high");
  });
  ...
  it("severe — any index reaches 5", () => {
    expect(
      getPollenRisk({ treeIndex: 0, grassIndex: 0, weedIndex: 5, moldIndex: 0 })
        .risk,
    ).toBe("severe");
  });

  it("uses the max index across all four categories", () => {
    const result = getPollenRisk({
      treeIndex: 1,
      grassIndex: 2,
      weedIndex: 5,
      moldIndex: 3,
