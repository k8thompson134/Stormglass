 ...
 * `aqiThreshold`, answering "when is it safe to go outside, and for how long" rather
 ...
 *
 * `aqiThreshold` is a raw US AQI value (0-500) -- contrast with
 * findCategoryClearTime's `categoryIdxThreshold`, which is a CATEGORY_ORDER index.
 * Passing one where the other is expected type-checks silently, so the names
...
export function findSafeWindows(
  ...
  let groupEnd: number | null = null; // index of last point at-or-below aqiThreshold
  ...
  const flush = () => {
    ...
    const safePoints = fullSpan.filter((p) => p.usAqi <= aqiThreshold);
    ...
    // averaged away.
    const avgAqi =
      safePoints.reduce((s, p) => s + p.usAqi, 0) / safePoints.length;
    const maxAqi = Math.max(...fullSpan.map((p) => p.usAqi));
    const durationHours =
      (endPt.timestamp.getTime() - startPt.timestamp.getTime()) / 3_600_000;
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (point.usAqi <= aqiThreshold) {
      ...
      const gapHours =
        (point.timestamp.getTime() - prevTs.getTime()) / 3_600_000;
...
export function nextSafeWindow(
  windows: SafeWindow[],
  now: Date,
): SafeWindow | null {
  ...
  return upcoming.reduce(
    (soonest, w) => (w.start < soonest.start ? w : soonest),
    upcoming[0],
  );
}

// The official EPA/AirNow US AQI category bands. getAQIRisk (backend) uses this
// directly; frontend/src/utils/aqiCategory.ts hand-copies the same boundaries
// (see that file's comment) -- keep the two in sync if these change.
...
export function getCategoryCeiling(category: AqiCategory): number {
...
export function effectiveAqi(
  modelAqi: number,
  hyperlocalAqi: number | null | undefined,
): number {
...
export function aqiScoreBump(usAqi: number | null | undefined): 0 | 1 | 2 {
  ...
  if (category === "Unhealthy for Sensitive Groups" || category === "Unhealthy")
    return 1;
  return 0;
}
