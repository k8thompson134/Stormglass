...
// import.
function parseGuidanceEntries(source: string, exportName: string): Record<string, string> {
  const blockMatch = source.match(new RegExp(`${exportName}[^{]*\\{([\\s\\S]*?)\\n\\};`));
  if (!blockMatch) throw new Error(`Could not find ${exportName} block`);

  const entryPattern = /(['"])((?:\\.|(?!\1).)*)\1\s*:\s*(['"])((?:\\.|(?!\3).)*)\3/g;
...
describe('CATEGORY_GUIDANCE stays in sync with the frontend copy', () => {
  it('has identical wording (modulo capitalization/punctuation) for every category', () => {
    const backendSource = readFileSync(resolve(__dirname, './push.ts'), 'utf-8');
    const frontendSource = readFileSync(
      resolve(__dirname, '../../../frontend/src/utils/aqiCategory.ts'),
      'utf-8'
    );

    const backendEntries = parseGuidanceEntries(backendSource, 'CATEGORY_GUIDANCE');
    const frontendEntries = parseGuidanceEntries(frontendSource, 'CATEGORY_GUIDANCE');

    expect(Object.keys(backendEntries).sort()).toEqual(Object.keys(frontendEntries).sort());
