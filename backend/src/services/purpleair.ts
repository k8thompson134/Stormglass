...
let cache: { key: string; result: HyperlocalAQI | null; expiresAt: number } | null = null;
...
function purpleAirEpaCorrection(pm25Cf1: number, humidity: number, tempF: number): number {
    const corrected = 0.541 * pm25Cf1 - 0.0618 * humidity + 0.00534 * tempF + 3.634;
...
async function fetchHyperlocalAQIUncached(
    ...
    try {
        ...
        if (!response.ok) {
            logger.warn({ service: 'purpleair', status: response.status }, 'PurpleAir API error');
            return null;
        }
        ...
        const candidates: { distance: number; pmRaw: number; humidity: number; tempF: number }[] = [];

        for (const row of rows) {
            const locationType = idx['location_type'] !== undefined ? row[idx['location_type']] : null;
            if (locationType !== 0) continue; // only outdoor sensors -- indoor sensors read totally different air

            const lastSeen = idx['last_seen'] !== undefined ? row[idx['last_seen']] : null;
            if (!lastSeen || nowSec - lastSeen > 3600) continue; // stale/offline sensor

            const confidence = idx['confidence'] !== undefined ? row[idx['confidence']] : 100;
            if (confidence !== null && confidence < 70) continue;

            const slat = idx['latitude'] !== undefined ? row[idx['latitude']] : null;
            const slng = idx['longitude'] !== undefined ? row[idx['longitude']] : null;
            if (slat === null || slng === null) continue;

            const pmA = idx['pm2.5_cf_1_a'] !== undefined ? row[idx['pm2.5_cf_1_a']] : null;
            const pmB = idx['pm2.5_cf_1_b'] !== undefined ? row[idx['pm2.5_cf_1_b']] : null;
            const pmRaw = pmA !== null && pmB !== null
                ? (pmA + pmB) / 2
                : (idx['pm2.5_cf_1'] !== undefined ? row[idx['pm2.5_cf_1']] : null);

            const humidity = idx['humidity'] !== undefined ? row[idx['humidity']] : null;
            const tempF = idx['temperature'] !== undefined ? row[idx['temperature']] : null;
        ...
        const avgHumidity = nearest.reduce((s, c) => s + c.humidity, 0) / nearest.length;
        const avgTempF = nearest.reduce((s, c) => s + c.tempF, 0) / nearest.length;

        const pm25 = Math.round(purpleAirEpaCorrection(avgPmRaw, avgHumidity, avgTempF) * 10) / 10;
        ...
        logger.error({ service: 'purpleair', err: error }, 'PurpleAir fetch failed');
        return null;
    }
}
